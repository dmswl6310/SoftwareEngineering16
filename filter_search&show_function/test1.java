...
public class test1 extends JPanel {
	...
	public test1() {
		...
		Label t1 = new Label(Language.getString("resName1"));//"식당이름");
		Label t2 = new Label(Language.getString("res_type"));//"분류");
		Label t3 = new Label(Language.getString("res_location"));//"위치");
		Label t4 = new Label(Language.getString("visit_time"));//"방문시각");
		Label t5 = new Label(Language.getString("price"));//"가격");
	...
	public static void fileScan() {
		...
		try {
			...
			System.out.println(Language.getString("fileCantopen"));//"파일을 열지 못합니다");
		} catch (NumberFormatException e) {
			System.out.println(Language.getString("wrongInput"));//"입력형식 맞지않음");
		}

		// 원하는 정보를 입력받는다.
		System.out.println(Language.getString("enterResInfo"));//">> Restaurant형 정보를 입력하세요");

		System.out.println(Language.getString("resName1"));//"식당 이름은?");
		x2.name = iscan.next();
		System.out.println(Language.getString("resType1"));//"원하는 분류는? (1:한식, 2:일식, 3:양식 , 4:중식, 5:기타)");
		x2.type = iscan.nextInt();
		System.out.println(Language.getString("resLoc"));//원하는 위치는? (1:정문(쪽문), 2:북문, 3:동문, 4:서문)");
		x2.location = iscan.nextInt();
		System.out.println(Language.getString("eatTime"));//"원하는 시간은?");
		x2.start_time = iscan.nextInt();
		x2.finish_time = x2.start_time;
		System.out.println(Language.getString("price"));//"원하는 가격은?");
		...
		System.out.println(Language.getString("relevantResInfo"));//"해당되는 식당정보는..");
